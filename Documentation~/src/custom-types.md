...
1. Create a new class that derives from `RuntimeReference<T, TVar>` where `T` should be the type you wish to wrap and `TVar` is the variable type.
...
1. Create a new class that derives from `RuntimeVariableWatcherEditor<T, TVar>`, where `T` should be the type and `TVar` is the variable type.
